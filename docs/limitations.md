...
Final-200 的结果支持：成功轨迹的 Action-only SFT 是本实验的主要能力增益；Process 选择没有优于 Outcome；在 `lr=1e-6`、LoRA `r=16`、`n=4` 的配方下，GRPO 未产生可检测增益。冻结合同是 `total_training_steps=500`、`save_freq=50`，本次由内部 exact-step barrier 在 100 optimizer steps 后受控停止并选择 step50；这里的 GRPO 结论是配方和评测协议限定的观察，不是对所有 GRPO 训练的普遍判断。
...
4. **训练搜索空间很窄。** GRPO 合同是 500 total training steps、`save_freq=50`，本次在 100 optimizer steps 受控停止；实验只考察一个学习率、LoRA rank 16 和每 prompt 四条 rollout，M3 为 step50 导出。该配方下未观察到增益，不说明更长训练、更大学习率或全参数训练的结果。
